...
const DetailedListingView = ({ property, onBack }) => {
  const { workplace, transportMode, detailedViewTab, setDetailedViewTab, selectedRoutes, setSelectedRoutes, selectedRouteIndex, setSelectedRouteIndex, favorites, toggleFavorite } = useAppContext();
              ...
              {route.steps.map((step, stepIndex) => (
                <div key={stepIndex} className="flex gap-3">
                  ...
                  <div className={`flex-shrink-0 mt-0.5 ${
                    ...
                    step.travelMode === 'TRANSIT' && step.transit?.line?.color
                      ? { backgroundColor: `#${step.transit.line.color}` }
                      : {}
                  }>
                    {getTravelModeIcon(step.travelMode, step.transit?.line?.vehicle?.type)}
                  </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            ...
                            style={{
                              backgroundColor: step.transit.line.color ? `#${step.transit.line.color}` : '#666',
                              color: step.transit.line.textColor ? `#${step.transit.line.textColor}` : '#fff'
                            }}
                          >
                        ...
                        <div className="text-sm">
                          <div className="text-gray-700">
                            {step.transit.headsign && (
                              <span className="font-medium">toward {step.transit.headsign}</span>
                            )}
                          </div>
                          <div className="text-xs text-gray-600 mt-1">
                            From <span className="font-medium">{step.transit.departureStop.name}</span>
                          </div>
                          <div className="text-xs text-gray-600">
                            To <span className="font-medium">{step.transit.arrivalStop.name}</span>
                          </div>
                          <div className="text-xs text-gray-500 mt-1">
                            {step.transit.numStops} {step.transit.numStops === 1 ? 'stop' : 'stops'} • {step.duration}
                          </div>
                        </div>
                      </div>
                    ) : (
